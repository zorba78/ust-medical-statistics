...
👉 Add up all the values in the sample, then divide by how many values you have.
...
📌 This is more than just a metaphor. Mathematically, the arithmetic mean is the point at which the sum of the distances to all data points, weighted by their values, is minimized.
...
📍 What value splits the data exactly in half?
...
🔹 First, suppose we have 5 observations:
{6, 2, 4, 3, 8}
...
🔹 On the left, we have a unimodal distribution.
There’s a single clear peak—one value that dominates in frequency. That’s the mode.
🔹 On the right, we see a bimodal distribution.
...
📌 The appearance of these modes depends heavily on how we choose the bin size in the histogram.
...
📌 How far, on average, are the data points from the mean?
...
📌 This is the Coefficient of Variation, or CV.
...
👉 We use the Coefficient of Variation, which standardizes the spread relative to the mean.
...
📌 A distribution is said to be skewed when one of its tails is noticeably longer or heavier than the other.
...
🔹 On the left, we see a distribution that is skewed to the left, also called negatively skewed.
...
🔹 On the right, we have a distribution that is skewed to the right, or positively skewed.
...
This visual helps us understand a key point: 👉 In skewed distributions, the mean and median are no longer equal.
...
📊 A barplot is especially useful when we want to display univariate data grouped by categories.
...
👉 Do not remove zero from the y-axis.
...
📌 A bar plot is used for categorical data, while a histogram is designed for numerical, continuous variables.
...
Why? 👉 Because density allows us to compare distributions even when sample sizes differ.
...
Let’s break it down:
   ...
   - The box itself spans from the 25th percentile (Q₁) to the 75th percentile (Q₃).
...
📌 Suppose we have a dataset: $x_1, x_2, \ldots, x_n$
...
📌 A scatterplot allows us to examine how one variable behaves in relation to another.
...
Then we multiply these deviations:
   ...
   - In Quadrants 1 and 3, both deviations have the same sign → their product is positive
   👉 This indicates that both variables increase or decrease together
   - In Quadrants 2 and 4, the deviations have opposite signs → their product is negative
   👉 This suggests that one variable increases while the other decreases
...
This leads us directly to the idea of covariance. 📌 Covariance measures how two variables vary together.
...
📌 The Pearson correlation coefficient, denoted as \rho_{XY}, is a standardized measure of the linear relationship between two variables.
...
Why do we scale?
👉 Because covariance alone is affected by the units of measurement.
...
📌 The correlation coefficient \rho quantifies the strength and direction of a linear relationship between two variables.
