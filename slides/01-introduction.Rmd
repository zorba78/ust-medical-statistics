...
Lots of people get nervous as soon as they hear the word ‘statistics.’ Mainly because most folks just don’t like numbers. And to be fair, numbers have fooled people plenty of times. Take all those economic figures you see in the news—they look super official, but the truth is, they’re often twisted, not to show some objective fact, but to push someone’s biased opinion or agenda.
...
The term 'frequentist" may be unfamiliar to you. Traditionally, there are two major schools of statistics: frequentist and Bayesian. The main difference between the two is the view point of the parameter. As mentioned on the slide, frequentists regard parameters as fixed values, whereas Bayesians consider parameters as random variables that have their own probability distributions. 

Suppose all existant apples on the planet consist of two colors: red and green and 
there exist huge size of box that could contain all of them. What we're curious about is the proportion of red apples in the box. that is, the true value of the parameter. 

So in this example, let me show you how the two approaches think about the parameter in different ways.


For a frequentist, the parameter θ (the proportion of red apples) is a fixed but unknown value.

👉 Whether the box has 40% or 60% red apples is already determined, but we just don’t know the exact number.

So, we take a sample (pick a few apples) and estimate that proportion.

Here, probability comes only from the randomness of sampling.
For example: “The probability that I pick a red apple is 40%.”

For a Bayesian, the parameter θ itself is considered uncertain.

👉 The proportion of red apples could be 40%, 60%, or many other possibilities, and we represent that uncertainty as a belief distribution.

We start with a prior belief (prior distribution).
(e.g., “In most orchards, about half the apples are red, so θ=0.5 is quite likely.”)

Then we draw a sample and update our belief using that information.
This gives us the posterior probability distribution for θ.
So, we might say: “Now, there’s a high probability that θ is around 0.6.”
