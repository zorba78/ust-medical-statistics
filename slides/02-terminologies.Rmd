...
In this class, we will focus on essential terms that you need to know. Additionally, when you actually analyze data, there's an important initial step you should always have to do, which is examining the descriptive characteristics of the data.
...
When we talk about statistics, there are generally two main activities.
The first is called exploratory data analysis. In this step, we describe what we see in the data: we compute simple numbers such as averages, percentages, or correlations, and we create plots and tables. The goal here is to get an overview of the data and to spot patterns, unusual cases, or potential errors. Think of this step as “looking at a picture” of your data.
...
The second activity is statistical inference. Here, we go further. We take a sample—a smaller piece of data we collected—and we use it to draw conclusions about a much larger group, the population. Inference lets us answer questions such as: 
...
Inference is like making an educated guess about the unseen world using the data we do see.
...
In statistics, we always distinguish between the population and the sample.
The population is the complete group we care about: all people, all patients, all schools, or all measurements of interest. 
...
For example, if the purpose of our study is to investigate the prevalence of dementia among the elderly in South Korea, the whole population of more than 65 year-old elders in South Korea is our target population of our study.
...
Unfortunately, it is usually impossible to measure the whole population.

That is why we collect a sample. A sample is a smaller group, carefully chosen from the population, that we actually observe. We hope this sample reflects the larger population well enough so that we can learn about the population without measuring everyone.

The sample is a subset of the population, and if we actually surveyed 500 elders in the study we designed, these 500 adults becomes your sample for the study. The action to collector extract sample is called "sampling". After sampling, you measure something like age, education level, several questionnaires and the prevalence of the dementia from these samples. These things become your dataset. In most case, sampling and collecting data are done simultaneously. 
...
A parameter is a numerical summary that describes a population.
It could be the true average income of a country, the true proportion of people who have diabetes, or the true mean blood pressure of all adults. The key idea is that the parameter is a fixed value, but it is unknown to us because we cannot measure the entire population.

Instead, we collect data from a sample and compute numbers from that data. These numbers are called sample statistics. For example, we may compute the average income of 1,000 surveyed households. That statistic is our estimate of the true population parameter.

So the parameter is like the “truth hidden in the background,” and statistics are our “best guesses” based on the information we have.
...
Most datasets are organized as a table. Mathematically, data is a matrix. 


The matrix is made up of rows and columns. Each row represents one unit of observation: this could be a person, a hospital, or a time point. Everything that you are interested in your research or study. The number of these indivduals determines the size of the sample.

For example, in a health dataset, each row could be one patient, and the columns could include variables such as age, sex, height, weight, and blood pressure.


Each column represents a variable, which is a characteristic measured on those units.

A variable is something measured from an individual, which can characterize that individual. 
...
Each tree living in this mountain is an individual. Species, height, and circumference at chest height of the tree will be variables that express the characteristics of the tree.


So when you look at a dataset, you can always ask two questions: “Who or what are the rows?” and “What attributes or measurements do the columns represent?”
...
Variables come in different types, and this classification matters because it determines which statistical methods we can use.
...
**Nominal variables** are categories without order, such as blood type or eye color.

The only operation we can do on these nominal variables is simply counting. 


**Ordinal variables** are categories with order, such as education level (high school < college < graduate school) or satisfaction ratings (low < medium < high).
...
**Interval variables** are numerical variables that measure relative concepts. Therefore, the concept of the absolute zero does not exist. The typical examples of interval variable are temperature and IQ. 
...
In short: variables differ in their structure, and we must respect this structure when analyzing data.



In addition, Variables of the highest concept can be converted to any of the lower level of variables (interval, ordinal, nominal). 
...
**Random variable** is a variable whose value is not known in advance but follows some chance mechanism.
...
Let’s start by clarifying two closely related but different concepts: **error** and **uncertainty**.

**Uncertainty** refers to our lack of knowledge. For instance, if we estimate the average blood pressure in a population from a sample, we are uncertain about how close our estimate is to the truth.

**Uncertainty** expresses how sure we are about our measurement. It is the range of possible values the true value might take given the limitations of our measurement. This is about what we don’t know yet.


**Error** is the difference between the “true value” and what we actually measured. It has already occurred and is a factual difference.


Key point: Error is **fact**, uncertainty is **belief** or **confidence**.


Let’s take a simple example: measuring body weight.

The error is straightforward: measured minus true → 61.2 – 60.0 = +1.2 kg. This is a fact that has already occurred.

The uncertainty comes from the scale’s limited precision. Since the scale’s accuracy is ±0.5 kg, the true weight could be anywhere between 60.7 and 61.7 kg.

Notice the difference: error is a specific known difference; uncertainty is a range of plausible values for the true weight.



**Variability** is the natural fluctuation in data. Even if we measure the same person’s blood pressure multiple times, the values will vary slightly.


<!-- Error refers to mistakes or deviations that occur during measurement or estimation. For example, a scale might read 1 kg higher than the true weight. -->




In statistics, we always deal with these three concepts. Our goal is to quantify uncertainty, reduce error, and explain variability.
...
Probability is the language we use to describe uncertainty.

When we say that the probability of rolling a 6 on a fair die is 1/6, we mean that if we roll the die many times, about one-sixth of the rolls will show a 6. Probability allows us to assign numbers to uncertain events in a consistent way. 
...
In statistics, probability is essential because it provides the foundation for making inferences. Without probability, we could not connect the random sample we observe to the larger population we care about.
...
The following is an example of a coin toss using R. This example illustrates the classical definition of probability in statistics very clearly.

Even if the proportion of heads appears noticeably higher or lower than 0.5 in a small number of trials, as the number of trials increases, this proportion tends to converge to 0.5. This can be observed through the simulation results.

The R script for this example is very simple, so it is recommended to review it after class and try running it yourself to see how it works.
...
Every random variable follows a probability distribution, which describes how likely different outcomes are.

For a coin toss, the distribution is simple: 50% heads and 50% tails. For human height, the distribution might look like a bell-shaped curve, where most people are around the average height, and very tall or very short people are less common.

Probability distributions allow us to describe uncertainty mathematically and to calculate the chances of various events. They are the building blocks of statistical inference.
...
The expectation (or expected value) of a random variable is its long-run average.

Imagine flipping a fair coin: the expected number of heads in one flip is 0.5, because if we flip many, many times, the average proportion of heads will get closer and closer to 50%.

Similarly, the expected height of a population is just the population mean. Expectation is a powerful concept because it summarizes what outcome we “expect” on average if we repeat an experiment indefinitely.
...
Because we cannot know population parameters directly, we rely on sample statistics. These are numbers we calculate from our data, such as the sample mean, the sample proportion, or the sample standard deviation.

One of the common misunderstanding about a sample is that the obtained sample is the only sample in the world.

In fact, in statistics, the obtained sample from a certain research is assumed to be one chosen by chance from a large set of samples. 

However, since samples vary from one to another, sample statistics are themselves random variables. One sample might give us an average weight of 65 kg, another might give 66 kg. This variation is natural and expected.
...
To capture this uncertainty, we use concepts like the standard error, which measures how much a statistic tends to vary across repeated samples. When we report statistics, it is best practice to present both the estimate and its uncertainty. 
...
There are two main tasks in statistical inference.

Estimation: We use data to estimate an unknown parameter. For example, “What is the average cholesterol level in this population?”

Estimation gives us numerical guesses, while hypothesis testing answers yes/no questions about differences or effects. Both are tools to connect sample data with population-level questions.


Hypothesis testing: We use data to test a claim about a parameter. For example, “Is the average cholesterol level different between men and women?”
...
The basic principles of estimation & hypothesis testing are almost identical to ALL types of parameters and statistics. 

However, there are many details. For example: there are many different methods to obtain the test statistic & estimate the standard error.

Depending on the difference in the type or structure of random variables, the probability distribution used to describe the random variable may also differ.
...
The process of statistical inference can be summarized in a flow.

First, we have a population with unknown parameters. We take a sample from this population. From the sample, we compute statistics such as means or proportions. These statistics follow their own sampling distributions. By understanding those distributions, we can estimate parameters and test hypotheses.

So the scheme is: Population → Sample → Statistics → Inference.
This cycle is at the heart of statistics, and once you see this structure, everything else in statistical methods can be viewed as a variation of this same idea.
